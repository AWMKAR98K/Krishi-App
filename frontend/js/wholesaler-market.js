...
document.getElementById('productForm').addEventListener('submit', async (e) => {
    ...
    try {
        const response = await fetch('http://localhost:5000/api/market/add-product', {
            method: 'POST',
...
async function loadWholesalerOrders() {
    try {
        ...
        const res = await fetch('http://localhost:5000/api/market/seller/orders/all');
...
async function verifyOrder(orderId) {
    ...
    try {
        const res = await fetch('http://localhost:5000/api/market/verify-order', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
